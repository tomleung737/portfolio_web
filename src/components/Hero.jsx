import { motion } from "framer-motion";

import { styles } from "../styles";
import { EarthCanvas } from "./canvas";

const Hero = () => {
  return (
    <section className="relative w-full h-screen mx-auto">
      <div className={`${styles.paddingX} absolute inset-0 top-[120px] max-w-7xl
      mx-auto flex flex-row items-start gap-5`}>
        <div className="flex flex-col justify-center items-center mt-5">
          <div className="w-5 h-5 rounded-full bg-primary" />
          <div className="w-1 sm:h-80 h-40 primary-gradient" />
        </div>

        <div>
          <h1 className={`${styles.heroHeadText} text-white`}>
            Hi, Here is <span className="text-primary">Tom</span>
          </h1>
          <p className={`${styles.heroSubText} mt-2 text-text`}>
            I am enthusiastic about web development. <br />
            I am also interested in UI/UX design and AI technologies.
          </p>
        </div>
      </div>

      <EarthCanvas />

      
      <div className="absolute xs:bottom-10 bottom-32 w-full flex flex-col 
        justify-center items-center"
      >
        <div className="mb-5">Click to scroll down</div>
        <a href="#about">
          <div className="w-[32px] h-[64px] rounded-3xl border-4 border-white flex
            justitfy-center items-start p-2">
            <motion.div 
              animate={{
                y: [0, 24, 0]
              }}
              transition={{
                duration: 1.5,
                repeat: Infinity,
                repeatType: 'loop',
              }}
              className="w-3 h-3 rounded-full bg-white mb-1" />
          </div>
        </a>
      </div>
    </section>
  )
}

export default Hero
